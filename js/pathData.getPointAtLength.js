...
/**
 * standalone replacement for native methods
 * getTotalLength() and getPointAtLength()
 * 
 * Usage:
 * 1. parse  d pathdata string to lookup object used for point at length calculation       
 * let pathLengthLookup = getPathLengthLookup(d, tDivisions);
 * 
 * 2. get total length
 * let pathLengthTotal = pathLengthLookup.totalLength;
 * 
 * 3. get point at length (e.g at 50%)
 * let pt1 = getPointAtLengthLookup(pathLengthLookup, pathLengthTotal / 50 );
 * 
 */
...
function getPathLengthLookup(d, tDivisions = 10) {
    // parse d attribute string to calculatable data array
    ...
    for (let i = 0; i < segments.length && !foundSegment; i++) {
        ...
        if (segment.end > length) {
            ...
            if (type === "L") {
                ...
                // calculate points
                pt = getPointAtCubicSegmentT(points[0], points[1], points[2], points[3], newT);
            }
        }
    }
    return pt;
 ...
 */
function calcSegmentLength(com, p0, points, pathLength = 0, tDivisions = 10) {
    let len, cp1, cp2, p, t;
...
function pathDataArcsAndQuadraticToCubic(pathData) {
    ...
    pathData.forEach((com, i) => {
        ...
        // convert quadratic to cubic
            ...
            pathDataNorm.push({ type: "C", values: [cp1.x, cp1.y, cp2.x, cp2.y, values[2], values[3]] });
        }
        else {
